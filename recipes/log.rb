namespace :log do
  desc 'Tail logs. Optionally pass name of log you want to check - e.g. cap util:logs mongrel. Defaults to #{environment}.log'
  task :default, :roles => :app do
    ...
    log_file = ARGV[2] ? ARGV[2] : environment
  ...
  desc 'Tail just the hits and errors; pass name of log if not #{environment}.log'
  ...
  task :rotate, :roles => :app do
    rotate_script = <<-EOF
#{shared_path}/log/#{environment}.log {
  daily
  rotate #{ENV['days'] || 7}
  size #{ENV['size'] || "5M"}
  compress
